public class Converter {
	...
	public static String toPostFix(String infixString){
		...
		StackVector<String> operatorStack = new StackVector<String>();
		...
		for(int i = 0; i < parsedInfix.length; i++){
			...
			if(parsedInfix[i] != null){
				...
				if(isNumeric(parsedInfix[i])){
					...
					while(!operatorStack.empty() && weight(operatorStack.peek()) >= weight(parsedInfix[i])){
	...
	private static String[] parse(char[] input) {
	    ...
	    StackVector<String> expStack = new StackVector<String>();
